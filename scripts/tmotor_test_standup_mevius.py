...
def main():
    ...
    for cur_step in range(step_num):
        for motor_id, motor_controller in motors.items():
            tgt_deg = start_angles_deg[motor_id-1] + (end_angles_deg[motor_id-1] - start_angles_deg[motor_id-1])*(cur_step/step_num)
    ...
    while(True):
        ...
        for motor_id, motor_controller in motors.items():
            ...
            tgt_deg = start_angles_deg[motor_id-1] + (end_angles_deg[motor_id-1] - start_angles_deg[motor_id-1])*(cur_step/step_num) + motor_offset_deg
            pos, vel, cur, tem = motor_controller.send_deg_command(tgt_deg, 0, args.kp, args.kd, 0)
            print(tgt_deg)
            print("Moving Motor {} Position: {}, Velocity: {}, Torque: {}, Temp: {}".format(motor_id, pos, vel, cur, tem))
