...
def test_wrap_to_pi():
    ...
    torch.testing.assert_close(wrap_to_pi(torch.tensor(2.1 * torch.pi)), torch.tensor(0.1 * torch.pi))
    torch.testing.assert_close(wrap_to_pi(torch.tensor(-2.1 * torch.pi)), torch.tensor(-0.1 * torch.pi))
    torch.testing.assert_close(wrap_to_pi(torch.tensor(torch.pi + 1e-6)), torch.tensor(-torch.pi + 1e-6))
    torch.testing.assert_close(wrap_to_pi(torch.tensor(-torch.pi)), torch.tensor(torch.pi)) # Add test for -np.pi explicitly
