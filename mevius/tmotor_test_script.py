...
def main():
    ...
    if args.task == 'pos':
        ...
        while not rospy.is_shutdown():
            for i, (motor_id, motor_controller) in enumerate(motors.items()):
                command_cur = max(
                    -4.0,
                    min(
                        0.02 * (pos_vec[(i + 1) % 2] - pos_vec[i])
                        + 0.01 * (vel_vec[(i + 1) % 2] - vel_vec[i]),
                        4.0,
                    ),
                )
        ...
        for deg in np.linspace(0.0, 360.0, int(args.time * args.hz / len(args.ids))):
            for i, (motor_id, motor_controller) in enumerate(motors.items()):
                pos, vel, cur, tem = motor_controller.send_deg_command(
                    initial_pos[i] + deg, 0, args.kp, args.kd, 0
                )
                print(
                    'Moving Motor {} Position: {}, Velocity: {}, Torque: {}, Temp: {}'.format(
                        motor_id, pos, vel, cur, tem
        ...
        for deg_rate in np.linspace(0.0, 1.0, int(move_time * args.hz / len(args.ids))):
            for i, (motor_id, motor_controller) in enumerate(motors.items()):
                command = final_angle[motor_id - 1] + deg_rate * (
                    initial_offset[motor_id - 1] - final_angle[motor_id - 1]
                )
                pos, vel, cur, tem = motor_controller.send_rad_command(
                    command, 0, args.kp, args.kd, 0
                )
